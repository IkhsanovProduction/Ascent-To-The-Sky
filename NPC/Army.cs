...
public class Army : NPC
{
    ...
    public void CheckForDistance()
    {
        ...
        {
            _isDistanceWithCharacter = true;
            _speed = 0;
            _animator.Play("Idle_Guard_AR");
            transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(GameManager.Instance.Player.transform.position - transform.position), 2* Time.deltaTime);
        }

        else
        {
            _isDistanceWithCharacter = false;
            _speed = 2;
        }
    ...
    private bool _isDistanceWithCharacter = false;

    public override void Move()
    {
        CheckForDistance();

        if (_isCanWalk && _isDistanceWithCharacter == false)
        {
            transform.position = Vector3.MoveTowards(transform.position, _moveSpot.position, _speed * Time.deltaTime);
            _animator.Play("WalkFront_Shoot_AR");
